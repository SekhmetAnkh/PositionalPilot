...
The local positional action map mirrors RotationSolverReborn's melee positional table for DRG, MNK, NIN, RPR, SAM, and VPR. Fresh known next-GCD positionals can identify committed Rear/Flank intent, but committed movement is blocked unless a movement budget can be evaluated safely. Unknown action IDs fall back to rear/flank border hold and never trigger NoCasting.
...
- Committed Rear/Flank movement probes a small ordered set of candidate destinations, but still issues at most one vnavmesh request per update.
...
- Fresh known RSR next-GCD/next-action positional changes or fresh known Wrath local predictions can bypass the repath cooldown once, but committed movement still requires a safe destination and a passing movement-budget decision.
...
- Fresh known RotationSolver next-GCD or next-action positionals select the movement slice, but the current RSR event IPC does not expose GCD remaining, action-ahead timing, or primary target id. Until reliable timing/target IPC exists, committed movement fails closed and may use the conservative True North fallback when all gates pass.
